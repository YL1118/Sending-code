...
- --caption-key：caption 的欄位名稱（預設 caption）。
- --shuffle：輸出前打亂列順序。
"""
...
def build_rows(file_paths: List[str], label: str, text_keys: List[str], concat_keys: List[str],
               caption_key: str, keep_caption_col: bool, min_chars: int, limit: int) -> List[Dict[str, str]]:
    ...
    for fp in file_paths:
        ...
        caption_val = ""
        if isinstance(obj, dict) and caption_key in obj:
            v = obj.get(caption_key)
            if isinstance(v, str):
                caption_val = v.strip()
            else:
                tmp = try_extract_text(v, [])
                caption_val = (tmp or "").strip()
        ...
        if not text:
            ...
            tk = [k for k in text_keys if not (keep_caption_col and k == caption_key)]
...
def build_split(root: str,
    ...
    for label in class_dirs:
        ...
        train_rows = build_rows(train_files, label, text_keys, concat_keys, caption_key,
                                keep_caption_col, min_chars, limit_per_class_train)
        test_rows = build_rows(test_files, label, text_keys, concat_keys, caption_key,
...
def main():
    ...
    ap.add_argument("--caption-key", default="caption", help="caption 對應的 JSON 欄位名（預設 caption）")
